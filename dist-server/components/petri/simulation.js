...
function createCreature(index, generation = 1) {
    return {
        id: id('creature'),
...
function maybeReproduce(snapshot) {
    ...
    if (cluster.length < 3 || Math.random() > 0.014 || next.creatures.length > 18)
        return next;
